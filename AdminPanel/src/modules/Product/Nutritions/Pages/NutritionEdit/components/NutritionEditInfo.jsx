...
export default function NutritionEditInfo({ extraInfo, setExtraInfo }) {
    const initializeRows = (info) =>
        info.map((row, index) => ({ id: row.id || Date.now() + index, ...row }));

    const [rows, setRows] = useState(initializeRows(extraInfo));
